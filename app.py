...
def generate_master_summary():
    """Generates a combined master health report summary based on session state data."""
    summary_data = []
    ...
    if not summary_data:
        return "No health metrics or assessment data recorded yet in this session. Use the tools available to generate results."
...
with st.sidebar:
    ...
    # EXACT ORDER: Home -> AI Command Center -> AI Symptom Tracker -> ...
    nav_options = [
        "Home",
        "AI Command Center",
        "AI Symptom Tracker",
...
# =========================================================
# PAGE ROUTING & CONTENT RENDERING
# =========================================================
...
if st.session_state.page == "Home":
    ...
    # TOP POSITION ON DASHBOARD: AI COMMAND CENTER (FIRST)
    tool(
        "🤖",
        "AI Command Center",
    ...
    # SECOND POSITION ON DASHBOARD: AI SYMPTOM TRACKER (SECOND)
    tool(
        "🩺",
        "AI Symptom Tracker",
...
# ---------------------------------------------------------
# 2. AI COMMAND CENTER (Placed directly above AI Symptom Tracker)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
    ...
    with st.container(border=True):
        ...
        selected_model = st.selectbox("Gemini Model:", ["gemini-2.5-flash", "gemini-2.5-pro"], index=0)
